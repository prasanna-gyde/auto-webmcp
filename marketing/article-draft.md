...
*Cross-post on dev.to, Hashnode, and LinkedIn. Also use as Show HN description.*
...
Chrome 146 shipped an Early Preview of **WebMCP** — a browser-native API that lets websites register their interactive elements as structured tools. The same idea as MCP (Model Context Protocol) for desktop AI, but built directly into the browser itself.
...
Here's what it takes to register a single form manually:
...
|--------------|----------------|
| Tool name | `toolname` attr → submit button text → nearest heading → form `id` → form `action` |
| Description | `tooldescription` attr → `<legend>` text → `aria-label` → page title |
| Field type | `<input type>`, `<select>`, `<textarea>` → JSON Schema |
| Field title | `<label>` text → `data-webmcp-title` → humanized field name |
| Options | `<option>` elements → `enum` + `oneOf` with display labels |
...
<form
  ...
  tooldescription="Search for available flights. Requires origin, destination, and departure date."
  toolautosubmit="true"
>
```

Or use `data-webmcp-*` attributes — useful for platforms where you can't touch the form element directly.

## Safe to ship on any site today

auto-webmcp is built around **progressive enhancement**:

- If `navigator.modelContext` is absent (any browser except Chrome 146+ with the flag), the script loads and silently does nothing
...
You can drop it into production today and regular users won't notice anything. Only WebMCP-capable agents will see the registered tools.

## Works on every platform

Because it's just a script tag:

- **WordPress**: install the [AI-Agent Ready Forms plugin](https://autowebmcp.dev/platforms/wordpress) or add one line to `functions.php`
...
3. Go to [autowebmcp.dev/demo](https://autowebmcp.dev/demo) — you'll see the registered tools appear in the inspector panel in real time

The agentic web is arriving. WebMCP is the infrastructure layer. auto-webmcp is the one-line on-ramp.

**[autowebmcp.dev](https://autowebmcp.dev) · [GitHub](https://github.com/prasanna-gyde/auto-webmcp) · MIT**

---

*Tags: webmcp, ai-agents, javascript, chrome, open-source, forms, automation*
